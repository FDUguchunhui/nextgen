...
class StatisticianAgent:
    ...
    def analyze(
        ...
        try:
            ...
            2. Compute metrics for each protein:
            3. Perform a statistical test (e.g., two-sample t-test) to compare expression levels between the two cancer types.
            4. Calculate the ROC AUC for each protein using using roc_auc_score from sklearn.metrics
            5. Output the number of samples in each group.
            5. Compute the log2-fold change between group means.
            6. Return a summary DataFrame with the following columns: 'protein_group', 'mean_intensity_{group1}', 'mean_intensity_{group2}', 'log2_fold_change', 'number_of_samples_{group1}', 'number_of_samples_{group2}', 'p_val', 'auc'.
            7. Sort the results by 'uac' in descending order and 'p_val' in ascending order.
            8. Suppress an warning message.
            '''
...
if __name__ == "__main__":
    ...
    question = (
    '''
    Perform a two-sample t-test for each unique protein comparing expression levels between the two cancer types breast and other.
    '''
)
    # question = "How many proteins are there in the data?"
    statistician = StatisticianAgent()
    response = statistician.analyze(df, question)
    print("\nResponse:", response)