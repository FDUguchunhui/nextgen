#!/usr/bin/env python3
"""
Script to create a proteins table in the SQLite database and populate it with data
from a TSV file.
"""
...
def create_database_schema(columns: List[str]) -> str:
    ...
    for col in columns:
        ...
        clean_col = col.replace(' ', '_').replace('(', '').replace(')', '').replace('[', '').replace(']', '').replace('-', '_').lower()
        # Remove multiple underscores
        clean_col = '_'.join(filter(None, clean_col.split('_')))
        clean_columns.append(clean_col)
    
    # Make Entry the primary key (now 'entry' in lowercase)
    sql_columns = []
...
def create_proteins_table(db_file: str, tsv_file: str) -> bool:
    """
    Create and populate the proteins table in the database.
    
    Args:
        db_file (str): Path to the SQLite database file
        tsv_file (str): Path to the TSV file containing protein data
    ...
    try:
        ...
        conn.close()
        print(f"Proteins table created and populated successfully!")
        return True
        
    except Exception as e:
        print(f"Error creating proteins table: {e}")
        return False

if __name__ == "__main__":
    create_proteins_table('../database/nextgen.db', '../data/human_canonical_proteins.tsv')